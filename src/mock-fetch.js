/**
 * This is based off the core of jest-fetch-mock, copyright (c) 2017 Jeff Lau
 * https://github.com/jefflau/jest-fetch-mock
 * 
 * Changes:
 *   - Use @dollarshaveclub/cloudworker's fetch instead of cross-fetch
 *   - Replace jest.fn calls with a bound module mock
 */
const cloudWorker = require('@dollarshaveclub/cloudworker')
actualFetch = cloudWorker.fetch
const Response = cloudWorker.Response
const Headers = cloudWorker.Headers
const Request = cloudWorker.Request
...
const isMocking = jestFn(staticMatches(true))
...
const normalizeResponse = (bodyOrFunction, init) => (input, reqInit) => {
  ...
  return mocked
    ...
    : actualFetch.fetch(input, reqInit)
}
...
const fetch = jestFn(normalizeResponse(''))
